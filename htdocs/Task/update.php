...
$nameerror = $passworderror = $phoneerror = $emailerror = $gendererror = $courseerror = $checkerror  = $multiselecterror = $languageerror =  $mainmsg = $imgerror= $imagedata="";
$namedata = $passworddata = $phonenumberdata = $emaildata = $coursedata = $genderdata = $multiselectdata = $languagedata = $new_languagedata= $skilldata= $mainerror= $image="";
...
if($img != ''){
  ...
  $flag +=1;
    ...
    $imgerror = 'file format is not supported';
  }
}
else{
  $imagedata =$old_image;
...
$sql = "UPDATE student_tb SET student_name = '$namedata ',email_id ='$emaildata' ,password ='$passworddata',phone_number =$phonenumberdata,gender='$genderdata',course ='$coursedata',skill ='$multiselectdata',language ='$languagedata',image ='$image' WHERE id= $seller_id";

  $flag =0;
  if($conn->query($sql) === TRUE){
$msg ="sucessfully updated";

header('Location:old_dashboard.php?msg=1');
}
else{
  echo 'not updated';
}
